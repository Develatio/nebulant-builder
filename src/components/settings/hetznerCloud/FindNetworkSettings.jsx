...
const network_filters = { type: "hetznerCloud:network" };

export const FindNetworkSettings = (props) => {
  const dql = new DiagramQL();

  const dqlResults = useOneShotState(() => ({
    networks: dql.query(
      dql.vars_for_dropdown({ node: props.node, ...network_filters })
    ),
  }));

  return (
    <Container>
      <WHeader help={props.help}>Hetzner Cloud - Find network</WHeader>
      ...
      <WBody>
            ...
            <Col sm={12}>
              <Nav variant="pills d-flex justify-content-center align-items-center stylish-tabs">
                <div className="tabsWrapper d-flex">
                  ...
                  {
                    props.node.prop("data/id") === "find-network" ? (
              ...
              <Row className="my-3">
                <Col sm={6}>
                  <TextInput
                    ...
                    help_text={"The name that will be used to filter the network."}
                  ></TextInput>
                </Col>
        ...
        <AdvancedSettings>
          {
            props.node.prop("data/id") === "find-networks" ? (
              <Paging {...props} />
            ) : ""
          }
