...
describe("Testing 'outputVars' command", () => {
  ...
  it("retrieves outputVars", () => {
    const res = dql.query(`nodes | outputVars`);
    assert.equal(typeof res, "object");
    assert.equal(res.length, 7);
  });
});

describe("Testing 'outputVars' command in an only Start box diagram", () => {
  const model = new BaseDiagramModel();
  runtime.set("objects.main_model", model);
  const dql = new DiagramQL(model);
